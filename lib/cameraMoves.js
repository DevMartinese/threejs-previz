...
export const moves = {
  ...
  /** 15b. Bezier fly: a CURVED slide (quadratic, `via` is the bend), look held
   * on a target — the fly-through that has to swerve around something. */
  bezier({ from = [-1, 1.5, 6], via = [0, 1.5, 3], to = [1, 1.5, 6],
           target = [0, 1, 0], fov = 45 } = {}) {
    return (u) => {
      const a = (1 - u) * (1 - u), b = 2 * (1 - u) * u, c = u * u;
      return {
        position: [0, 1, 2].map((k) => a * from[k] + b * via[k] + c * to[k]),
        target, fov, roll: 0,
      };
    };
  },
...
/**
 * FLOAT SPIN as a pure function of time: the subject turns and bobs, the
 * camera holds. `t` is seconds — `frame / fps` — and every value is computed
 * from it absolutely.
 *
 * It used to take a `dt` and accumulate (`rotation.y += speed * dt`), which
 * is exactly what this pipeline forbids: called twice on one frame it
 * double-rotates, and Remotion's out-of-order workers would each see a
 * different scene. Absolute-from-t is the only form that survives.
 */
export function floatSpin(object, t, { speed = (Math.PI * 2) / 5, bobAmp = 0.1,
                                       bobFreq = 0.8, baseY = object.position.y } = {}) {
  object.rotation.y = speed * t;
  if (bobAmp) object.position.y = baseY + Math.sin(t * bobFreq * Math.PI * 2) * bobAmp;
}

/** 15. Tracking follow: hold an offset to a moving subject with damping. Call each frame. */
/**
 * TRACKING / FOLLOW as a move. `subjectAt(u) -> [x,y,z]` is the subject's own
 * path, so the shot is a function of u like every other move.
 *
 * The telephoto lag that a damped follow gives you is here without any
 * damping state: the camera sits at the subject's position `lag` earlier
 * along the path, and aims `lookAhead` later. Stiff damping is a small lag,
 * loose damping a large one. The previous version lerped from the camera's
 * OWN last position, which meant the shot depended on which frame had been
 * rendered before it — invisible in a live loop, fatal across workers.
 */
export function follow(subjectAt, { offset = [0, 2, 8], lag = 0.04,
                                    lookAhead = 0, fov = 45 } = {}) {
  return (u) => {
    const behind = subjectAt(Math.max(0, u - lag));
    const ahead = subjectAt(Math.min(1, u + lookAhead));
    return {
      position: [behind[0] + offset[0], behind[1] + offset[1], behind[2] + offset[2]],
      target: ahead, fov, roll: 0,
    };
  };
}
