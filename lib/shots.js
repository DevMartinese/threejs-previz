...
function auditContinuity(scene, camera, list, shot,
  ...
  for (let f = shot.from; f < shot.to; f++) {
    ...
    camUp.push(new Vector3(0, 1, 0).applyQuaternion(camera.quaternion));
  ...
  // path.
  // ROLL is measured too, not just position and view direction. A camera
  // aimed exactly along its own up axis (straight down, straight up) hits
  // the lookAt singularity: the view direction stays smooth while the frame
  // SPINS — a 178-degree up-vector flip in one frame reads on screen as
  // badly as any teleport, and nothing else here would catch it. Keep a
  // steep gaze a few degrees off vertical.
  const camJumps = [];
  const steps = [], angs = [], ups = [];
  for (let i = 1; i < camPos.length; i++) {
    steps.push(camPos[i].distanceTo(camPos[i - 1]));
    angs.push((camDir[i].angleTo(camDir[i - 1]) * 180) / Math.PI);
    ups.push((camUp[i].angleTo(camUp[i - 1]) * 180) / Math.PI);
  }
  const median = (a) => a.length ? [...a].sort((x, y) => x - y)[Math.floor(a.length / 2)] : 0;
  const medStep = median(steps), medAng = median(angs), medUp = median(ups);
  for (let i = 0; i < steps.length; i++) {
    const badPos = steps[i] > Math.max(5 * medStep, subjectSize * 0.05);
    const badAng = angs[i] > Math.max(5 * medAng, 15);
    const badUp = ups[i] > Math.max(5 * medUp, 15);
    if ((badPos || badAng || badUp) && camJumps.length < 8) {
      camJumps.push({ frame: shot.from + i + 1,
                      step: steps[i], angle: angs[i], roll: ups[i] });
    }
  }
  const cameraPath = {
  ...
  return report.map((r) => {
    ...
    if (r.continuity && r.continuity.cameraPath && r.continuity.cameraPath.jumps.length) {
      bits.push(`camera ${r.continuity.cameraPath.jumps.map((j) =>
        `jump ${j.step.toFixed(3)}m/${j.angle.toFixed(1)}deg`
        + (j.roll > 15 ? `/roll ${j.roll.toFixed(0)}deg` : '')
        + `@f${j.frame}`).join(',')}`);
    }
    if (r.join && !r.join.ok) {
