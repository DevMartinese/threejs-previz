...
export function auditShots(scene, camera, list, {
  ...
  floorIgnore = [], subjectSize = 1, clearance = null,
} = {}) {
  const report = [];
  ...
  for (const shot of list.shots) {
    let framing = null, floor = null, occlusion = null, clear = null;
    const collisions = [];
    ...
    for (const f of list.samplesFor(shot, samples)) {
      ...
      const fl = auditFloor(meshes, { ignore: floorIgnore });
    ...
    report.push({
      ...
      hero: shot.hero || null,
      framing, floor, collisions, occlusion, clearance: clear, continuity,
...
function auditContinuity(scene, camera, list, shot,
  ...
  return { pops, teleports, ok: pops.length === 0 && teleports.length === 0 };
}
