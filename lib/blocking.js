...
import {
  Box3, BoxGeometry, Color, DirectionalLight, DoubleSide, Frustum, Group,
  HemisphereLight, Matrix4, Mesh, MeshBasicMaterial, MeshLambertMaterial, Object3D,
  PerspectiveCamera, Ray, Scene, Sphere, Vector3,
} from 'three';
...
export function createBlocking({
  ...
  const ctx = {
    ...
    /**
     * The WORLD position of a local-space point on a part — "the cup's mouth",
     * "the moka's spout" — computed through the scene graph with the part's
     * current pose, rotation and scale included. Attach things to anchors,
     * never to hand-computed trigonometry: a pour authored as `anchor(cup,
     * mouth)` stays on the mouth when the cup's tilt changes; a hand-typed
     * constant silently pours out of the belly.
     */
    anchor(name, local = [0, 0, 0]) {
      const o = ctx.get(name);
      o.updateWorldMatrix(true, false);
      return o.localToWorld(new Vector3(...local));
    },
...
/* =========================================================================
 * The bounding-box audits — collisions, framing, floor
 *
 ...
 * `ignore` (names or wildcards) declares intentional breaches — a ball that
 * plunges into the water, a keel below the waterline — so they are documented
 * rather than silenced by moving the plane.
 */
export function auditFloor(objects, { y = 0, tolerance = 1e-3, ignore = [] } = {}) {
  const skip = ignore.map(globToRe);
  let min = Infinity, who = null;
  for (const o of objects) {
    if (!o.isMesh || !o.visible) continue;
    if (skip.some((re) => re.test(o.name))) continue;
    o.updateWorldMatrix(true, false);
    const b = new Box3().setFromObject(o);
...
/* =========================================================================
 * Sightline audits — occlusion, camera clearance, frustum
 *
 * The bbox audits above answer "does it fit"; these answer "can you SEE it"
 * and "is the camera somewhere legal". All exact, all BVH, all headless.
 * ======================================================================= */
...
/** First world-space hit distance of camera->point against one mesh, or null. */
function hitDistance(mesh, origin, point) {
  const bvh = ensureBVH(mesh);
  _ray.origin.copy(origin);
  _ray.direction.copy(point).sub(origin).normalize();
  mesh.updateWorldMatrix(true, false);
  _inv.copy(mesh.matrixWorld).invert();
  const local = _ray.clone().applyMatrix4(_inv);
  const hit = bvh.raycastFirst(local, DoubleSide);
  if (!hit) return null;
  return _pt.copy(hit.point).applyMatrix4(mesh.matrixWorld).distanceTo(origin);
}

/**
 * What fraction of a mesh is hidden behind other meshes, seen from a camera.
 *
 * Casts rays from the camera to five sample points of the target's world AABB
 * (centre, top, bottom, and the two sides across the camera's right axis) and
 * counts the rays something else intercepts first. `0` = fully visible.
 *
 * This is the measurable form of "nobody blocks cyan at the end": the framing
 * audit proves the hero is inside the frame, this one proves you can see it.
 */
export function auditOcclusion(target, blockers, camera, { samples = null } = {}) {
  target.updateWorldMatrix(true, false);
  camera.updateMatrixWorld(true);
  const box = new Box3().setFromObject(target);
  const c = box.getCenter(new Vector3());
  const right = new Vector3().setFromMatrixColumn(camera.matrixWorld, 0);
  const rXZ = Math.hypot(box.max.x - box.min.x, box.max.z - box.min.z) / 2;
  const pts = samples ?? [
    c.clone(),
    new Vector3(c.x, box.min.y + (box.max.y - box.min.y) * 0.9, c.z),
    new Vector3(c.x, box.min.y + (box.max.y - box.min.y) * 0.1, c.z),
    c.clone().addScaledVector(right, rXZ * 0.7),
    c.clone().addScaledVector(right, -rXZ * 0.7),
  ];
  const origin = camera.getWorldPosition(new Vector3());
  // never let the target, its ancestors or its children block itself
  const family = new Set();
  target.traverse((o) => family.add(o));
  for (let p = target.parent; p; p = p.parent) family.add(p);

  let blocked = 0;
  let worst = null;
  for (const p of pts) {
    const dTarget = origin.distanceTo(p);
    for (const b of blockers) {
      if (!b.isMesh || !b.visible || family.has(b)) continue;
      const d = hitDistance(b, origin, p);
      if (d !== null && d < dTarget - 1e-4) { blocked++; worst = b.name; break; }
    }
  }
  const fraction = blocked / pts.length;
  return { fraction, blocker: worst, ok: true };   // ok is decided by the caller's threshold
}

/**
 * Exact distance from the camera to the nearest visible mesh, via BVH
 * closest-point — NOT bounding boxes, so a camera inside a room or a glass
 * measures its distance to the walls, not to their box. Catches the camera
 * clipping through geometry: anything closer than the near plane renders as a
 * hole cut through the object, and a camera inside a solid renders garbage.
 */
export function auditCameraClearance(meshes, camera, { min = null } = {}) {
  const origin = camera.getWorldPosition(new Vector3());
  const floor = min ?? camera.near;
  let worst = { distance: Infinity, object: null };
  for (const m of meshes) {
    if (!m.isMesh || !m.visible) continue;
    const bvh = ensureBVH(m);
    m.updateWorldMatrix(true, false);
    _inv.copy(m.matrixWorld).invert();
    const local = origin.clone().applyMatrix4(_inv);
    const hit = bvh.closestPointToPoint(local);
    if (!hit) continue;
    const d = _pt.copy(hit.point).applyMatrix4(m.matrixWorld).distanceTo(origin);
    if (d < worst.distance) worst = { distance: d, object: m.name };
  }
  return { ...worst, min: floor, ok: worst.distance >= floor };
}
...
/** Is any of this mesh inside the camera's view frustum right now? */
export function inFrustum(mesh, camera) {
  camera.updateMatrixWorld(true);
  mesh.updateWorldMatrix(true, false);
  _proj.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse);
  _frustum.setFromProjectionMatrix(_proj);
  new Box3().setFromObject(mesh).getBoundingSphere(_sphere);
  return _frustum.intersectsSphere(_sphere);
}

/** Is this mesh's centre hidden behind any of the blockers, from this camera? */
export function isOccluded(mesh, blockers, camera) {
  const r = auditOcclusion(mesh, blockers, camera, {
    samples: [new Box3().setFromObject(mesh).getCenter(new Vector3())],
  });
  return r.fraction > 0;
}
