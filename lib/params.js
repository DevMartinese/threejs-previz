...
export function paramSource(schema, values, indent = '  ') {
  const lines = Object.values(schema).map((d) => {
    ...
    const parts = [`value: ${typeof v === 'string' ? JSON.stringify(v) : v}`];
    if (d.kind === 'number') {
      parts.push(`min: ${d.min}`, `max: ${d.max}`, `step: ${d.step}`);
      if (d.unit) parts.push(`unit: ${JSON.stringify(d.unit)}`);
    }
    if (d.kind === 'enum') parts.push(`options: [${d.options.map((o) => JSON.stringify(o)).join(', ')}]`);
    if (d.note) parts.push(`note: ${JSON.stringify(d.note)}`);
    return `${indent}  ${d.key}: { ${parts.join(', ')} },`;
  });
