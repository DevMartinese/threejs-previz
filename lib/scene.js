...
export function defineScene({
  ...
  function make() {
    ...
    // A timeline is BUILT ONCE here, next to the geometry, and only ever
    // seeked afterwards. It is duck-typed — anything with `.seek(ms)` works,
    // there is no dependency on a particular library; anime.js is simply the
    // one measured to qualify (see test/timeline-determinism.mjs). That is what keeps it inside the determinism
    // contract: `.seek()` was measured to be independent of call order —
    // identical results across sequential, shuffled and reverse seeking, for
    // absolute, relative, [from,to] and implicit-from values alike — so
    // Remotion's out-of-order workers get the same frame every time. Build it
    // per frame instead and that guarantee is gone.
    if (timeline) ctx.timeline = timeline({ ctx, blk, geo, fps });
    return ctx;
  }
  ...
  function pose(ctx, frame) {
    if (ctx.timeline) ctx.timeline.seek((frame / fps) * 1000);
  ...
  function audit({ samples = 6 } = {}) {
    ...
    const onFrame = (animate || timeline) ? (f) => pose(ctx, f) : null;
  ...
  return {
    ...
    list, visibility, ignore, exclude, animate, timeline,
    make, audit, pose,
    /**
