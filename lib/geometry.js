...
import {
  ...
  DodecahedronGeometry, ExtrudeGeometry, IcosahedronGeometry, InstancedMesh,
  LatheGeometry, Object3D, OctahedronGeometry, Path, Quaternion,
  RingGeometry, Shape, SphereGeometry, TetrahedronGeometry, TorusGeometry,
...
/**
 * A capsule — pill, ladyfinger, limb segment, cocktail olive. Y-aligned;
 * total height is `length + 2r`. Watertight, so it cuts cleanly.
 */
export function capsule({ r = 0.01, length = 0.03, capSegments = 8, radialSegments = 16 } = {}) {
  return new CapsuleGeometry(r, length, capSegments, radialSegments);
}

/**
 * A box with rounded corners — product blocking's other workhorse: phones,
 * cards, soap bars, remotes, packaging. Built the way drei's RoundedBox is
 * (a rounded rect extruded with a bevel of the same radius), so it stays
 * watertight and CSG-safe. `r` is the corner radius; `smoothness` the curve
 * segment count.
 */
export function roundedBox({ x = 1, y = 1, z = 1, r = 0.05, smoothness = 4 } = {}) {
  const eps = 1e-5;
  const rr = Math.min(r, Math.min(x, y, z) / 2 - eps);
  const w = x - rr * 2, h = y - rr * 2;
  const shape = new Shape();
  shape.absarc(eps, eps, eps, -Math.PI / 2, -Math.PI, true);
  shape.absarc(eps, h - eps * 2, eps, Math.PI, Math.PI / 2, true);
  shape.absarc(w - eps * 2, h - eps * 2, eps, Math.PI / 2, 0, true);
  shape.absarc(w - eps * 2, eps, eps, 0, -Math.PI / 2, true);
  const g = new ExtrudeGeometry(shape, {
    depth: z - rr * 2, bevelEnabled: true, bevelSegments: smoothness * 2,
    steps: 1, bevelSize: rr - eps, bevelThickness: rr, curveSegments: smoothness,
  });
  g.center();
...
/**
 * A low-poly platonic solid — rocks, gems, boulders, dice, faceted proxies.
 * `detail: 0` is the faceted look that reads as "rock" in blocking; pair with
 * `roughen()` for organic boulders. kinds: ico (20 faces), octa (8),
 * dodeca (12), tetra (4).
 */
export function gem({ kind = 'ico', radius = 0.5, detail = 0 } = {}) {
  const K = { ico: IcosahedronGeometry, octa: OctahedronGeometry,
              dodeca: DodecahedronGeometry, tetra: TetrahedronGeometry };
  if (!K[kind]) throw new Error(`gem: unknown kind "${kind}" (ico|octa|dodeca|tetra)`);
  return new K[kind](radius, detail);
}
