...
export function defineFilm({
  ...
  /**
   * The film's knobs are its scenes' knobs, keyed by scene id — nothing is
   * declared here. A film is an edit, not a thing with its own dimensions:
   * the values belong to the scenes and this only routes them.
   *
   *   { tiramisu: { hover: 0.11 }, orbital: { apex: 68 } }
   *
   * The film's DURATION cannot move, and that is free rather than checked
   * here: `defineScene` already refuses a parameter set that changes a scene's
   * length, so every scene keeps its frame count and the transition
   * arithmetic above stays valid whatever the knobs say.
   */
  const params = Object.fromEntries(
    scenes.filter((s) => Object.keys(s.params ?? {}).length).map((s) => [s.id, s.params]));

  /** Validate `{sceneId: {...}}` against each scene's own declaration. */
  function resolve(overrides = {}) {
    const out = {};
    for (const [sceneId, over] of Object.entries(overrides ?? {})) {
      const scene = scenes.find((s) => s.id === sceneId);
      if (!scene) {
        throw new Error(`film "${id}": no scene "${sceneId}" in this film`
          + ` — it has ${scenes.map((s) => s.id).join(', ')}`);
      }
      out[sceneId] = scene.resolve(over);   // throws on unknown key / bad range
    }
    return out;
  }

  /** Each scene's shot list under `overrides` — what the seam check measures. */
  function listsFor(overrides = {}) {
    const m = new Map();
    for (const s of scenes) {
      const over = overrides?.[s.id];
      m.set(s.id, over ? s.listFor(s.resolve(over)) : s.list);
    }
    return m;
  }
  ...
  return {
    ...
    params, resolve, listsFor,
    problems: check(scenes, transitions, seams),
    /**
     * Re-run the consistency checks. Used by the audit gate.
     *
     * `overrides` matters most for the seam check: a knob that moves a camera
     * moves where a scene ENDS, and a dive-through that spliced at the
     * defaults may not splice any more. That is exactly the failure this file
     * exists to catch, so it has to be measured against the values you are
     * actually about to render.
     */
    check: (overrides = null) =>
      check(scenes, transitions, seams, overrides ? listsFor(overrides) : null),
     ...
     */
    compositionProps: {
      id, durationInFrames, fps, width, height,
      // `--props='{"params":{"<sceneId>":{...}}}'` on the film render lands
      // here and is routed to each scene's own component.
      defaultProps: { params: {} },
    },
  };
}
 ...
 */
function check(scenes, transitions, seams, lists = null) {
  const problems = [];
  const { fps, width, height } = scenes[0];
  // The shot list to measure each scene by: its own, or the one a parameter
  // set implies. Everything below reads through this, so the checks never
  // measure the defaults while the render uses something else.
  const listOf = (s) => (lists ? lists.get(s.id) : s.list);
  ...
  if (seams) {
    ...
    const pose = (scene, frame) => {
      ...
      applyFrame(cam, listOf(scene), frame);
    ...
    for (let i = 0; i < scenes.length - 1; i++) {
      ...
      const a = pose(A, listOf(A).duration - 1);
      const b = pose(B, 0);
  ...
  for (const [after, t] of Object.entries(transitions)) {
    ...
    const shortest = Math.min(listOf(scenes[i]).duration, next ? listOf(next).duration : Infinity);
